...
&emsp;&emsp;上面4行信息是源代码提供的、以`bz2`压缩的用户名与密码。通过[脚本](src/8.py)解压，获得用户名`huge`与密码`file`，登录并进入下一题，后续密码也将使用相同的密码进行登录。
...
&emsp;&emsp;用[脚本](src/9.py)绘制图案，得到一头牛。重定向到`cow.html`，得到提示：
...
&emsp;&emsp;编写[脚本](src/10.py)以获得第30项的长度（**注意：此处使用数组下标来标记项，1是第0项**），运行得到结果
$$ \large len(a[30])=5808 $$
跳转到`5808.html`，进入下一关。
...
&emsp;&emsp;运行[脚本](src/11.py)，用`PIL`模块将2张图片的像素点分离。在显示出的黑色背景图片的右上角写有淡淡的`evil`字样，访问`evil.html`进入下一关。
...
&emsp;&emsp;运行[脚本](src/12.py)，得到5张图片，内容分别是`dis`、`pro`、`port`、`ional`、~~`ity`~~（**图五的3个字母被划去**），所以答案为`disproportial`。访问`disproportional.html`进入下一关。
...
&emsp;&emsp;编写并运行[脚本](src/13.py)，Bert给我们的回复是
...
&emsp;&emsp;编写并运行[脚本](src/14.py)，显示的结果图像是一只猫。访问`cat.html`，出现了一张完整清晰的图片，上面有2只猫，金色提示写着：
...
&emsp;&emsp;编写并运行[脚本](src/15.py)，求得年份为1756年。[查阅资料](https://baike.baidu.com/item/%E6%B2%83%E5%B0%94%E5%A4%AB%E5%86%88%C2%B7%E9%98%BF%E7%8E%9B%E5%A4%9A%E4%BC%8A%E6%96%AF%C2%B7%E8%8E%AB%E6%89%8E%E7%89%B9/6129936?fromtitle=%E8%8E%AB%E6%89%8E%E7%89%B9&fromid=159018)得知，当天是莫扎特（**Mozart**）的生日。访问`mozart.html`进入下一关。
